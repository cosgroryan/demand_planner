...
async def generate_parent_forecast(request: ForecastRequest):
    ...
    raise HTTPException(
        ...
        aggregated_data = matching_skus.group_by("Date").agg([
        ...
        # Add metadata
        forecast_result["series_id"] = f"{parent_sku}_parent"
        forecast_result["avg_selling_price"] = avg_selling_price
        forecast_result["total_skus"] = len(matching_skus)
        forecast_result["sku_list"] = matching_skus["SKU"].unique().to_list()
...
async def generate_parent_graphic_forecast(request: ForecastRequest):
    ...
    raise HTTPException(
        ...
        # Add metadata
        forecast_result["series_id"] = f"{parent_graphic_sku}_parent_graphic"
        forecast_result["avg_selling_price"] = avg_selling_price
        forecast_result["total_skus"] = len(matching_skus)
        forecast_result["sku_list"] = matching_skus["SKU"].unique().to_list()
